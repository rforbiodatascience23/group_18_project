...
::: columns
::: {.column width="50%"}
#### `dplyr` & `tidyverse` {.space-below}

`pivot_longer()`: data manipulation to transform the data from a wide to a longer format

`select()`: select specific variables

`rename()`: rename variables

`distinct()`: remove duplicate rows from the data
:::
...
The dataset under consideration originates from an in-depth exploration of medical records pertaining to individuals who underwent radiation therapy for prostate cancer within the timeframe of January 2000 to September 2015.
...
-   Talk about findins from the plots here.... 
