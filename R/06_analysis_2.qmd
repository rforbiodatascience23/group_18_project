...
### Plot 2.3 Correlation Analysis

Description: The plot depicts the correlation between chosen pairs of variables (above a selected threshold), allowing a simple understanding of their relationships. The intensity of the colour indicates the strength and direction of the correlation.
...
# Create a ggplot scatterplot with color representing correlation
correlation_plot <- 
...
ggsave(filename = "key_plot_2.3.png", 
...
cor(df$age_at_radiation, df$num_days_admitted)

# Make predictions
df <- df %>%
  mutate(predicted_days = predict(model, newdata = df))

# Create a scatterplot with the regression line
model_plot_days_admitted <-
ggplot(df, aes(x = age_at_radiation, y = num_comorbidities, color = predicted_days)) +
...
ggsave(filename = "key_plot_2.4.png", 
...
Residual plots are a crucial diagnostic tool to assess the validity of a linear regression model. They help you evaluate whether the assumptions of the model are met. Here are several types of residual plots you can create and interpret:
