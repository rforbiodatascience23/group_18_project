...
By doing a Principal Component Analysis in the data set we will reduce its dimmensionality and figure out if patients cancer organ have similar values in quantitative variables *Grading system, Radiation dosage, Duration of radiation, Number of commorbidities.*
...
selected_vars <- c("grading_system", "radiation_dosage", "duration_of_radiation_days", "num_comorbidities")

# Create a subset without NA values
hc_patients_clean <- hc_patients |> 
  select(all_of(selected_vars), cancer_organ) |> 
  drop_na() 

# Use the selected numeric columns for PCA and avoid rows with NA values
pca_fit <- hc_patients_clean |> 
...
```{r}
pca_biplot<- pca_fit |> 
  augment(hc_patients_clean) |>  # add original dataset back in
  drop_na() |>  
  ggplot(aes(.fittedPC1, .fittedPC2, color = cancer_organ)) + 
  geom_point(size = 2.5) +
  labs(color = "Cancer organ")+
  xlab("PC1") +
  ylab("PC2") +
  ggtitle("Principal Component Analysis of patients with cancer and HC") +
  scale_color_manual(
    values = c('prostate' = "#D55E00", 'prostate,bladder' = "#9900b2", 'bladder' = "#0066FF"),
    breaks = c('prostate', 'prostate,bladder', 'bladder'),  
...
Arrow plot. We should see one arrow per variable included in PCA, so 4. I don't understand it, **maybe we could remove this plot**
