...
clean_data1 <- clean_data1 |> 
  mutate(
    num_comorbidities = ifelse(is.na(comorbidities), 0, 
                               lengths(str_split(comorbidities, ','))
  ))
...
### Data Wrangling: Type of Radiation

Minimize the amount of different radiations and group the same ones together

```{r}
clean_data1 <- clean_data1 |> 
  mutate(new_radiation_type = case_when(
    grepl("EBRT", radiation_type) ~ "EBRT",
    grepl("IMRT", radiation_type) ~ "IMRT",
    TRUE ~ radiation_type))
```
