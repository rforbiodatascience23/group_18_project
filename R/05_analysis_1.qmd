...
Linear Regression Analysis of BMI in relation to radiation dose.
...
Description: Relationship Between Body Mass Index (BMI) and time from radiation completion to first incidence of Hemorrhagic Cystitis.
...
Plot 8: The distribution of radiation durations among different types of radiation.
...
faceted_scatter_plot <- ggplot(data, aes(x = radiation_dosage, y = radiation_duration, color = radiation_type)) +
  geom_point() +
...
# 3D Scatter Plot
scatter_3d <- plot_ly(data, x = ~radiation_dosage, y = ~radiation_duration, z = ~cancer_type, color = ~radiation_type, type = "scatter3d", mode = "markers") %>%
